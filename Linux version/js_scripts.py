...
JS_DEBUG_DOM = """
() => {
    function topClasses(selector, n) {
        return Array.from(document.querySelectorAll(selector))
            .slice(0, n)
            .map(function(e){
                return (e.className || "").toString().split(" ").slice(0,3).join(" ");
            });
    }
    return {
        title:      document.title,
        h3:         Array.from(document.querySelectorAll("h3")).map(function(e){ return e.textContent.trim(); }).slice(0,15),
        li_count:   document.querySelectorAll("li").length,
        li_classes: topClasses("li", 8),
        div_testid: Array.from(document.querySelectorAll("[data-testid]"))
